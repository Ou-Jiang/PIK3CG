...
# 确保元数据包含必要的列
if (!all(c("merged_type", "Residual.Tumor_group") %in% colnames(sce@meta.data))) {
  stop("元数据中缺少 'merged_type' 或 'Residual.Tumor_group' 列")
}
...
for (merged_type in merged_types) {
  ...
  # 检验组间差异
  expr_by_group <- split(subset_obj$gene_expression, subset_obj$Residual.Tumor_group)
  if (length(expr_by_group) == 2) {
    test_result <- t.test(expr_by_group[[1]], expr_by_group[[2]])
    p_value <- test_result$p.value
  } else {
    test_result <- kruskal.test(subset_obj$gene_expression ~ subset_obj$Residual.Tumor_group)
    p_value <- test_result$p.value
  }
  ...
  # 合并到结果列表
  results_list[[merged_type]] <- subset_data[, c("Cell_ID", "Residual.Tumor_group", "merged_type", "Gene_Expression", "Significance")]
}
...
p <- ggbarplot(dat2, x = "ImmuneCell", y = "Score", color = 'group',fill = NULL,
  ...
  scale_colour_manual(values = c(high="orange",low="navy"))+
  ...
  theme(axis.title.y = element_text(size = 20, face = "bold", family = "Times"),
   ...
   labs(title="PIK3CG")
ggsave(filename = "PIK3CG_Residual_tumor_group_cell_barplot.pdf", height = 5, width = 12)
ggsave(filename = "PIK3CG_Residual_tumor_group_cell_barplot.jpg", height = 5, width = 12)
